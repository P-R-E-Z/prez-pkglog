PROJECT := prez-pkglog
SPEC	:= $(PROJECT).spec
VERSION	:= $(shell grep '^Version' $(SPEC) | awk '{print $$2}')
SDIST	:= dist/$(PROJECT)-$(VERSION).tar.gz
MOCKCFG := fedora-42-x86_64
MOCKDIR := mock/$(MOCKCFG)

.PHONY: help lint test build sdist wheel srpm rpm mock install release clean
help:
	@echo "Targets: lint test build sdist wheel srpm rpm mock install release clean"

lint:
	uv run ruff check src/

test:
	uv run pytest -q

build: sdist wheel

sdist:
	python -m build --sditst

wheel:
	python -m build --wheel

srpm: sdist
	rpmbuild -ts $(SDIST)

rpm: srpm
	rpmbuild -ba $(SPEC)

mock: srpm
	mock -r $(MOCKCFG) ~/rpmbuild/SRPMS/$(PROJECT)-$(VERSION)-1*.src.rpm

install: rpm
	sudo dnf install -y ~/rpmbuild/RPMS/noarch/$(PROJECT)-$(VERSION)-1*.noarch.rpm

release:
	@test "$(v)" || (echo "usage: make release v=patch|minor|major" && exit 1)
	uv run bump2version $(v)
	git push && git push --tags

clean:
	rm -rf build dist *.egg-info
	find . -name '__pycache__' -exec rm -rf {} +
